...
from hermes_agent.cli import HermesCLI, _build_compact_banner, _rich_text_from_ansi
from hermes_agent.hermes_cli.skin_engine import get_active_skin, set_active_skin
...
class TestCliSkinPromptIntegration:
    ...
    def test_icon_only_skin_symbol_still_visible_in_special_states(self):
        ...
        with patch("hermes_agent.hermes_cli.skin_engine.get_active_prompt_symbol", return_value="⚔ "):
            assert cli._get_tui_prompt_fragments() == [("class:sudo-prompt", "🔑 ⚔ ")]
...
class TestCompactBannerSkinIntegration:
    def test_default_compact_banner_keeps_legacy_nous_hermes_branding(self):
        ...
        with patch("hermes_agent.cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \

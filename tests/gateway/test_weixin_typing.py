...
def weixin_adapter():
    ...
    from hermes_agent.gateway.platforms.weixin import WeixinAdapter, TypingTicketCache
...
class TestEnsureTypingTicket:
    ...
    async def test_refreshes_when_ticket_expired(self, weixin_adapter):
        ...
        with patch("hermes_agent.gateway.platforms.weixin._get_config", new_callable=AsyncMock) as mock_get:
    ...
    async def test_stop_typing_refreshes_ticket(self, weixin_adapter):
        ...
        with patch("hermes_agent.gateway.platforms.weixin._get_config", new_callable=AsyncMock) as mock_get, \
             patch("hermes_agent.gateway.platforms.weixin._send_typing", new_callable=AsyncMock) as mock_send:
            mock_get.return_value = mock_response
...
class TestTypingTicketCache:
    ...
    def test_returns_ticket_when_fresh(self):
        from hermes_agent.gateway.platforms.weixin import TypingTicketCache

...
class TestMatrixBangCommandAlias:
    ...
    def test_known_bang_command_normalizes_to_slash_command(self):
        from hermes_agent.gateway.platforms.matrix import _normalize_matrix_bang_command
...
class TestMatrixMarkdownToHtml:
    ...
    def test_matrix_markdown_rejects_obfuscated_javascript_links(self):
        from hermes_agent.gateway.platforms.matrix import _sanitize_matrix_html
...
class TestMatrixModuleImport:
    def test_module_importable_without_mautrix(self):
        """hermes_agent.gateway.platforms.matrix must be importable even when mautrix is
        ...
        result = subprocess.run(
            [sys.executable, "-c", (
                ...
                "with patch('hermes_agent.tools.lazy_deps.ensure', side_effect=ImportError('blocked')):\n"
...
class TestMatrixRequirements:
    def test_check_requirements_with_token(self, monkeypatch):
        ...
        with patch("hermes_agent.tools.lazy_deps.feature_missing", return_value=()):
    ...
    def test_check_requirements_encryption_true_no_e2ee_deps(self, monkeypatch):
        ...
        with patch.object(matrix_mod, "_check_e2ee_deps", return_value=False), \
             patch("hermes_agent.tools.lazy_deps.feature_missing", return_value=()):
    ...
    def test_check_requirements_e2ee_optional_no_deps_ok(self, monkeypatch):
        ...
        with patch.object(matrix_mod, "_check_e2ee_deps", return_value=False), \
             patch("hermes_agent.tools.lazy_deps.feature_missing", return_value=()), \
             patch("hermes_agent.tools.lazy_deps.ensure_and_bind", return_value=True):
    ...
    def test_check_e2ee_deps_requires_asyncpg(self, monkeypatch):
        ...
        """
        from hermes_agent.gateway.platforms.matrix import _check_e2ee_deps
        import builtins
    ...
    def test_check_requirements_runs_lazy_install_when_partial(self, monkeypatch):
        ...
        with patch("hermes_agent.tools.lazy_deps.feature_missing", return_value=("asyncpg==0.31.0",)), \
             patch("hermes_agent.tools.lazy_deps.ensure_and_bind", side_effect=_fake_ensure_and_bind):
...
class TestMatrixDiagnostics:
    ...
    def test_matrix_recovery_key_is_never_logged(self, caplog, monkeypatch):
        from hermes_agent.gateway.platforms.matrix import _handle_generated_matrix_recovery_key
    ...
    def test_capability_matrix_is_declared_for_docs(self):
        from hermes_agent.gateway.platforms.matrix import get_matrix_capabilities
    ...
    def test_matrix_capability_claims_match_adapter_surfaces(self):
        from hermes_agent.gateway.platforms.matrix import MatrixAdapter, get_matrix_capabilities
...
class TestMatrixClockSkewWarning:
    ...
    async def test_late_drops_emit_one_shot_clock_skew_warning(self, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="hermes_agent.gateway.platforms.matrix"):
            for i in range(5):
                ev = self._mk_event(
        ...
        skew_warnings = [
            ...
            if r.name == "hermes_agent.gateway.platforms.matrix"
...
class TestCreateMatrixSession:
    ...
    async def test_no_proxy_returns_trust_env_session(self):
        with patch.dict("sys.modules", _make_fake_mautrix()):
            from hermes_agent.gateway.platforms.matrix import _create_matrix_session

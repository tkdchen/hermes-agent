...
def _fresh_node_modules(bridge_dir: Path) -> None:
    ...
    from hermes_agent.gateway.platforms.whatsapp import _file_content_hash
...
class TestStaleBridgeHandshake:
    ...
    async def test_reuses_bridge_when_hash_matches(self, tmp_path):
        ...
        with patch("hermes_agent.gateway.platforms.whatsapp.check_whatsapp_requirements", return_value=True), \
             ...
             patch("hermes_agent.gateway.platforms.whatsapp.asyncio.create_task") as mock_task, \
    ...
    async def test_restarts_bridge_on_hash_mismatch(self, tmp_path):
        ...
        with patch("hermes_agent.gateway.platforms.whatsapp.check_whatsapp_requirements", return_value=True), \
             ...
             patch("hermes_agent.gateway.platforms.whatsapp._kill_stale_bridge_by_pidfile"), \
             patch("hermes_agent.gateway.platforms.whatsapp._kill_port_process") as mock_kill_port, \
    ...
    async def test_restarts_unversioned_bridge(self, tmp_path):
        ...
        with patch("hermes_agent.gateway.platforms.whatsapp.check_whatsapp_requirements", return_value=True), \
             ...
             patch("hermes_agent.gateway.platforms.whatsapp.asyncio.sleep", new_callable=AsyncMock), \
             patch("hermes_agent.gateway.platforms.whatsapp._kill_stale_bridge_by_pidfile"), \
             patch("hermes_agent.gateway.platforms.whatsapp._kill_port_process"), \
             patch("subprocess.Popen", return_value=mock_proc) as mock_popen, \
             patch.object(adapter, "_acquire_platform_lock", return_value=True, create=True):

...
async def test_reconnect_storm_sets_and_heartbeat_clears_flag(monkeypatch):
    ...
    monkeypatch.setattr(
        "hermes_agent.gateway.platforms.telegram.Update", MagicMock(ALL_TYPES=[])
    )

    await adapter._handle_polling_network_error(OSError("Bad Gateway"))
    assert adapter._send_path_degraded is True

    with patch("hermes_agent.gateway.platforms.telegram.asyncio.sleep", new_callable=AsyncMock):

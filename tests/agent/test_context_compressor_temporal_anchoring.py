...
import hermes_agent.hermes_time as hermes_time
from hermes_agent.agent.context_compressor import ContextCompressor, HISTORICAL_TASK_HEADING
...
def test_first_compaction_prompt_contains_dated_anchoring_rule():
    ...
    with patch.object(hermes_time, "now", _fixed_now), patch(
        "hermes_agent.agent.context_compressor.call_llm", return_value=_response("summary")
    ) as mock_call:
...
def test_iterative_update_prompt_also_contains_anchoring_rule():
    ...
    with patch.object(hermes_time, "now", _fixed_now), patch(
        "hermes_agent.agent.context_compressor.call_llm", return_value=_response("updated summary")
    ) as mock_call:
        compressor._generate_summary(_turns())

...
class TestReadClaudeCodeCredentialsFromKeychain:
    ...
    def test_returns_none_on_linux(self):
        ...
        with patch("hermes_agent.agent.anthropic_adapter.platform.system", return_value="Linux"):
            assert _read_claude_code_credentials_from_keychain() is None

    def test_returns_none_on_windows(self):
        with patch("hermes_agent.agent.anthropic_adapter.platform.system", return_value="Windows"):
            assert _read_claude_code_credentials_from_keychain() is None
    ...
    def test_returns_none_when_security_command_not_found(self):
        ...
        with patch("hermes_agent.agent.anthropic_adapter.platform.system", return_value="Darwin"), \
             patch("hermes_agent.agent.anthropic_adapter.subprocess.run",
    ...
    def test_returns_none_on_nonzero_exit_code(self):
        ...
        with patch("hermes_agent.agent.anthropic_adapter.platform.system", return_value="Darwin"), \
             patch("hermes_agent.agent.anthropic_adapter.subprocess.run") as mock_run:

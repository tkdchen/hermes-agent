...
class TestSkillMatchesPlatformTermux:
    ...
    def test_no_platforms_field_matches_everywhere(self):
        ...
        with patch("hermes_agent.agent.skill_utils.sys.platform", "android"), patch(
            "hermes_agent.agent.skill_utils.is_termux", return_value=True
        ):
    ...
    def test_linux_skill_loads_on_termux_linux_platform(self):
        ...
        fm = {"platforms": ["linux"]}
        with patch("hermes_agent.agent.skill_utils.sys.platform", "linux"), patch(
            "hermes_agent.agent.skill_utils.is_termux", return_value=True
        ):
            assert skill_matches_platform(fm) is True
    ...
    def test_non_termux_android_does_not_widen(self):
        ...
        fm = {"platforms": ["linux"]}
        with patch("hermes_agent.agent.skill_utils.sys.platform", "android"), patch(
            "hermes_agent.agent.skill_utils.is_termux", return_value=False
        ):
            assert skill_matches_platform(fm) is False
    ...
    def test_macos_skill_on_real_macos_unaffected(self):
        fm = {"platforms": ["macos"]}
        with patch("hermes_agent.agent.skill_utils.sys.platform", "darwin"), patch(
            "hermes_agent.agent.skill_utils.is_termux", return_value=False
        ):
            assert skill_matches_platform(fm) is True

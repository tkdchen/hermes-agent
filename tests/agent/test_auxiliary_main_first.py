...
class TestResolveAutoMainFirst:
    ...
    def test_openrouter_main_uses_main_model_for_aux(self, monkeypatch):
        ...
        with patch(
            "hermes_agent.agent.auxiliary_client._read_main_provider",
    ...
    def test_nous_main_uses_main_model_for_aux(self, monkeypatch):
        ...
        with patch(
            "hermes_agent.agent.auxiliary_client._read_main_provider", return_value="nous",
        ), patch(
            "hermes_agent.agent.auxiliary_client._read_main_model",
    ...
    def test_non_aggregator_main_still_uses_main(self, monkeypatch):
        ...
        with patch(
            "hermes_agent.agent.auxiliary_client._read_main_provider", return_value="deepseek",
        ), patch(
            "hermes_agent.agent.auxiliary_client._read_main_model", return_value="deepseek-chat",
        ), patch(
            "hermes_agent.agent.auxiliary_client.resolve_provider_client"
        ) as mock_resolve:
            mock_client = MagicMock()
    ...
    def test_main_unavailable_falls_through_to_chain(self, monkeypatch):
        ...
        with patch(
            "hermes_agent.agent.auxiliary_client._read_main_provider", return_value="anthropic",
        ), patch(
            "hermes_agent.agent.auxiliary_client._read_main_model", return_value="claude-opus",
        ), patch(
            "hermes_agent.agent.auxiliary_client.resolve_provider_client",
            return_value=(None, None),  # main provider has no client
        ), patch(
            "hermes_agent.agent.auxiliary_client._try_openrouter",
    ...
    def test_main_unavailable_uses_task_fallback_chain_before_builtin_chain(self):
        ...
        with patch(
            "hermes_agent.agent.auxiliary_client._read_main_provider", return_value="nvidia",
        ), patch(
            "hermes_agent.agent.auxiliary_client._read_main_model", return_value="qwen/qwen3.5-122b-a10b",
        ), patch(
            "hermes_agent.agent.auxiliary_client.resolve_provider_client",
            return_value=(None, None),  # main provider has no client
        ), patch(
            ...
            "hermes_agent.agent.auxiliary_client._try_main_fallback_chain",
    ...
    def test_no_main_config_uses_chain_directly(self):
        ...
        chain_client = MagicMock()
        with patch(
            "hermes_agent.agent.auxiliary_client._read_main_provider", return_value="",
        ), patch(
            "hermes_agent.agent.auxiliary_client._read_main_model", return_value="",
        ), patch(
            "hermes_agent.agent.auxiliary_client._try_openrouter",
            return_value=(chain_client, "google/gemini-3-flash-preview"),
        ):
    ...
    def test_runtime_override_wins_over_config(self, monkeypatch):
        ...
        with patch(
            ...
            "hermes_agent.agent.auxiliary_client._read_main_model", return_value="config-model",
        ), patch(
            "hermes_agent.agent.auxiliary_client.resolve_provider_client"
        ) as mock_resolve:
...
class TestResolveVisionMainFirst:
    ...
    def test_openrouter_main_vision_uses_main_model(self, monkeypatch):
        ...
        with patch(
            "hermes_agent.agent.auxiliary_client._read_main_provider", return_value="openrouter",
        ), patch(
            "hermes_agent.agent.auxiliary_client._read_main_model",
            return_value="anthropic/claude-sonnet-4.6",
        ), patch(
    ...
    def test_exotic_provider_with_vision_override_preserved(self):
        ...
        with patch(
            "hermes_agent.agent.auxiliary_client._read_main_provider", return_value="xiaomi",
        ), patch(
            "hermes_agent.agent.auxiliary_client._read_main_model",
    ...
    def test_copilot_vision_sets_vision_header(self, monkeypatch):
        ...
        with patch(
            "hermes_agent.agent.auxiliary_client._read_main_provider", return_value="copilot",
        ), patch(
            "hermes_agent.agent.auxiliary_client._read_main_model", return_value="configured-copilot-model",
        ), patch(
            "hermes_agent.agent.auxiliary_client._resolve_task_provider_model",
            return_value=("auto", None, None, None, None),
        ), patch(
            "hermes_agent.agent.auxiliary_client.OpenAI",
            ...
            "hermes_agent.hermes_cli.copilot_auth.copilot_request_headers",

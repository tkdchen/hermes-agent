    ...
    def _run_cleanup_isolated(self, cli_mod, **extra_patches):
        ...
        try:
            with (
                ...
                patch("hermes_agent.tools.mcp_tool.shutdown_mcp_servers", lambda *a, **k: None),
                patch(
                    "hermes_agent.agent.auxiliary_client.shutdown_cached_clients",
                    lambda *a, **k: None,
                ),
                patch("hermes_agent.hermes_cli.plugins.invoke_hook", lambda *a, **k: None),
            ):

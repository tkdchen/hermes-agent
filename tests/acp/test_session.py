...
from hermes_agent.acp_adapter import session as acp_session
from hermes_agent.acp_adapter.session import SessionManager, SessionState
...
class TestCreateSession:
    ...
    def test_create_session_registers_task_cwd(self, manager, monkeypatch):
        calls = []
        monkeypatch.setattr("hermes_agent.acp_adapter.session._register_task_cwd", lambda task_id, cwd: calls.append((task_id, cwd)))
        state = manager.create_session(cwd="/tmp/work")
    ...
    def test_register_task_cwd_translates_windows_drive_for_wsl_tools(self, monkeypatch):
        ...
        monkeypatch.setattr("hermes_agent.hermes_constants._wsl_detected", True)
        monkeypatch.setattr(
            "hermes_agent.tools.terminal_tool.register_task_env_overrides",
...
class TestWslCwdTranslation:
    ...
    def test_translate_acp_cwd_leaves_windows_drive_path_unchanged_off_wsl(self, monkeypatch):
        monkeypatch.setattr("hermes_agent.hermes_constants._wsl_detected", False)

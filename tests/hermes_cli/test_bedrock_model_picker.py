...
class TestProviderModelIdsBedrock:
    ...
    def test_returns_live_discovered_model_ids(self, monkeypatch):
        ...
        with patch("hermes_agent.agent.bedrock_adapter.discover_bedrock_models", side_effect=_mock_discover), \
             patch("hermes_agent.agent.bedrock_adapter.resolve_bedrock_region", return_value="eu-central-1"):
    ...
    def test_region_determines_model_ids(self, monkeypatch):
        ...
        with patch("hermes_agent.agent.bedrock_adapter.discover_bedrock_models", side_effect=_mock_discover):
            with patch("hermes_agent.agent.bedrock_adapter.resolve_bedrock_region", return_value="eu-central-1"):
                eu_result = provider_model_ids("bedrock")
            with patch("hermes_agent.agent.bedrock_adapter.resolve_bedrock_region", return_value="us-east-1"):
    ...
    def test_falls_back_to_static_list_when_discovery_empty(self, monkeypatch):
        ...
        with patch("hermes_agent.agent.bedrock_adapter.discover_bedrock_models", return_value=[]), \
             patch("hermes_agent.agent.bedrock_adapter.resolve_bedrock_region", return_value="eu-central-1"):
            result = provider_model_ids("bedrock")
    ...
    def test_falls_back_to_static_list_on_exception(self, monkeypatch):
        ...
        with patch("hermes_agent.agent.bedrock_adapter.discover_bedrock_models",
    ...
    def test_accepts_bedrock_aliases(self, monkeypatch):
        ...
        with patch("hermes_agent.agent.bedrock_adapter.discover_bedrock_models", side_effect=_mock_discover), \
             patch("hermes_agent.agent.bedrock_adapter.resolve_bedrock_region", return_value="us-east-1"):
...
class TestListAuthenticatedProvidersBedrock:
    ...
    def test_bedrock_appears_with_aws_profile(self, monkeypatch):
        ...
        with patch("hermes_agent.agent.bedrock_adapter.has_aws_credentials", return_value=True), \
             patch("hermes_agent.agent.bedrock_adapter.discover_bedrock_models", side_effect=_mock_discover), \
             patch("hermes_agent.agent.bedrock_adapter.resolve_bedrock_region", return_value="eu-central-1"):
    ...
    def test_bedrock_total_models_matches_discovery(self, monkeypatch):
        ...
        with patch("hermes_agent.agent.bedrock_adapter.has_aws_credentials", return_value=True), \
             patch("hermes_agent.agent.bedrock_adapter.discover_bedrock_models", return_value=_EU_MODELS), \
             patch("hermes_agent.agent.bedrock_adapter.resolve_bedrock_region", return_value="eu-central-1"):
    ...
    def test_bedrock_not_shown_without_credentials(self, monkeypatch):
        ...
        with patch("hermes_agent.agent.bedrock_adapter.has_aws_credentials", return_value=False):
            providers = list_authenticated_providers(current_provider="openai")
    ...
    def test_non_bedrock_picker_does_not_probe_full_aws_chain(self, monkeypatch):
        ...
        with patch("hermes_agent.agent.bedrock_adapter.has_aws_credentials", side_effect=_has_aws_credentials):
    ...
    def test_bedrock_falls_back_to_curated_when_discovery_fails(self, monkeypatch):
        ...
        with patch("hermes_agent.agent.bedrock_adapter.has_aws_credentials", return_value=True), \
             patch("hermes_agent.agent.bedrock_adapter.discover_bedrock_models",
...
class TestBedrockOverlayRegistration:
    ...
    def test_bedrock_aliases_resolve(self):
        from hermes_agent.hermes_cli.providers import normalize_provider

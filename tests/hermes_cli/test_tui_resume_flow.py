...
def main_mod(monkeypatch):
    import hermes_agent.hermes_cli.main as mod
...
def test_termux_fast_cli_launch_oneshot_uses_light_parser(monkeypatch, main_mod):
    ...
    monkeypatch.setitem(
        sys.modules,
        "hermes_agent.hermes_cli.oneshot",
        types.SimpleNamespace(
...
def test_termux_skips_bundled_skill_sync_when_stamp_fresh(monkeypatch, tmp_path, main_mod):
    ...
    monkeypatch.setitem(
        sys.modules,
        "hermes_agent.tools.skills_sync",
...
def test_oneshot_fails_closed_on_empty_final_response(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    import hermes_agent.hermes_cli.oneshot as oneshot_mod
...
def test_oneshot_filters_invalid_toolsets_before_redirect(monkeypatch, capsys):
    _stub_plugin_discovery(monkeypatch)
    from hermes_agent.hermes_cli.oneshot import _validate_explicit_toolsets
...
def test_oneshot_wires_session_db_for_recall(monkeypatch):
    ...
    from hermes_agent.hermes_cli.oneshot import _run_agent
    ...
    monkeypatch.setitem(sys.modules, "hermes_agent.run_agent", mod("hermes_agent.run_agent", AIAgent=FakeAgent))
    monkeypatch.setitem(sys.modules, "hermes_agent.hermes_state", mod("hermes_agent.hermes_state", SessionDB=FakeSessionDB))
    monkeypatch.setitem(
        sys.modules,
        "hermes_agent.hermes_cli.config",
        mod("hermes_agent.hermes_cli.config", load_config=lambda: {"model": {"default": "m"}}),
    )
    monkeypatch.setitem(
        sys.modules,
        "hermes_agent.hermes_cli.models",
        mod("hermes_agent.hermes_cli.models", detect_provider_for_model=lambda *_args, **_kwargs: None),
    )
    monkeypatch.setitem(
        sys.modules,
    ...
    monkeypatch.setitem(
        sys.modules,
        "hermes_agent.hermes_cli.tools_config",
        mod("hermes_agent.hermes_cli.tools_config", _get_platform_tools=lambda *_args, **_kwargs: {"session_search"}),
    )
...
def test_launch_tui_exit_code_42_relaunches_update(monkeypatch, main_mod):
    ...
    with patch("hermes_agent.hermes_cli.relaunch.relaunch") as mock_relaunch:
        with pytest.raises(SystemExit) as exc:
            main_mod._launch_tui()
...
def test_print_tui_exit_summary_includes_resume_and_token_totals(monkeypatch, capsys):
    ...
    monkeypatch.setitem(
        sys.modules, "hermes_agent.hermes_state", types.SimpleNamespace(SessionDB=lambda: _FakeDB())
    )

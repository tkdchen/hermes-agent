...
def test_parse_duration_accepts_formats():
    from hermes_agent.hermes_cli.kanban import _parse_duration
...
def test_dashboard_direct_status_change_off_running_closes_run(kanban_home):
    ...
    """
    from hermes_agent.plugins.kanban.dashboard.plugin_api import _set_status_direct
...
def test_cli_bulk_complete_with_summary_rejects(kanban_home):
    ...
    env = os.environ.copy()
    r = _run(
        [sys.executable, "-m", "hermes_agent.hermes_cli.main", "kanban",
...
def test_cli_create_on_fresh_home_auto_inits(tmp_path, monkeypatch):
    ...
    r = _sp.run(
        [_sys.executable, "-m", "hermes_agent.hermes_cli.main", "kanban",
...
def test_check_dispatcher_presence_warns_when_flag_off(monkeypatch):
    ...
    from hermes_agent.hermes_cli import kanban as kb_cli
    monkeypatch.setattr("hermes_agent.gateway.status.get_running_pid", lambda: 999)
    monkeypatch.setattr(
        "hermes_agent.hermes_cli.config.load_config",
...
def test_check_dispatcher_presence_silent_on_probe_error(monkeypatch):
    ...
    monkeypatch.setattr("hermes_agent.gateway.status.get_running_pid", _raise)
    running, msg = kb_cli._check_dispatcher_presence()
    assert running is True
    assert msg == ""
...
def test_cli_create_silent_when_gateway_up(kanban_home, monkeypatch, capsys):
    ...
    from hermes_agent.hermes_cli import kanban as kb_cli
    monkeypatch.setattr("hermes_agent.gateway.status.get_running_pid", lambda: 4242)
    monkeypatch.setattr(
        "hermes_agent.hermes_cli.config.load_config",
        lambda: {"kanban": {"dispatch_in_gateway": True}},
    )
...
def test_gateway_dispatcher_disables_corrupt_board_without_traceback(
    ...
    monkeypatch.setattr("hermes_agent.gateway.run.asyncio.to_thread", _to_thread)
    monkeypatch.setattr("hermes_agent.gateway.run.asyncio.sleep", _sleep)

    with caplog.at_level(logging.ERROR, logger="hermes_agent.gateway.run"):
        asyncio.run(
            asyncio.wait_for(
                runner._kanban_dispatcher_watcher(),
                timeout=3.0,
...
def test_gateway_dispatcher_retries_corrupt_board_after_quarantine(
    ...
    monkeypatch.setattr("hermes_agent.gateway.run.time.monotonic", _monotonic_for_gateway_dispatcher)
    monkeypatch.setattr("hermes_agent.gateway.kanban_watchers.time.monotonic", _monotonic_for_gateway_dispatcher)
    ...
    with caplog.at_level(logging.INFO, logger="hermes_agent.gateway.run"):
        asyncio.run(
            asyncio.wait_for(
                runner._kanban_dispatcher_watcher(),
                timeout=3.0,

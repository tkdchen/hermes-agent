...
from hermes_agent.hermes_cli.web_server import _save_anthropic_oauth_creds
...
def oauth_file(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr('hermes_agent.agent.anthropic_adapter._HERMES_OAUTH_FILE', target)
    monkeypatch.setattr('hermes_agent.agent.credential_pool.load_pool', lambda _provider: _DummyPool())
    return target
...
def test_dashboard_oauth_write_uses_atomic_replace_and_cleans_temp_files(oauth_file, monkeypatch):
    ...
    monkeypatch.setattr('hermes_agent.hermes_cli.web_server.os.replace', flaky_replace)

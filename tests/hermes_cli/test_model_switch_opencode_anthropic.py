...
def _run_opencode_switch(
    ...
    with (
        patch("hermes_agent.hermes_cli.model_switch.resolve_alias", return_value=None),
...
class TestAgentSwitchModelDefenseInDepth:
    """hermes_agent.run_agent.AIAgent.switch_model() also strips /v1 as defense-in-depth."""
    ...
    def test_agent_switch_model_strips_v1_for_anthropic_messages(self):
        ...
        with patch(
            ...
            "hermes_agent.agent.anthropic_adapter._is_oauth_token", return_value=False
        ):
        ...
        assert captured.get("base_url") == "https://opencode.ai/zen/go", (
            f"hermes_agent.agent.switch_model did not strip /v1; passed {captured.get('base_url')} "
...
class TestStaleConfigDefaultDoesNotWedgeResolver:
    ...
    def test_kimi_switch_keeps_v1_despite_claude_config_default(self, tmp_path, monkeypatch):
        ...
        import hermes_agent.hermes_cli.runtime_provider as _rp_mod
        importlib.reload(_rp_mod)
        import hermes_agent.hermes_cli.model_switch as _ms_mod

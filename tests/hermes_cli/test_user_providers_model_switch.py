...
import pytest
from hermes_agent.hermes_cli.model_switch import list_authenticated_providers, switch_model
from hermes_agent.hermes_cli import runtime_provider as rp
...
def test_list_authenticated_providers_uses_live_models_for_user_provider(monkeypatch):
    ...
    monkeypatch.setattr("hermes_agent.hermes_cli.models.fetch_api_models", fake_fetch_api_models)
...
def test_list_authenticated_providers_openai_alias_not_emitted_as_phantom(monkeypatch):
    ...
    monkeypatch.setattr(
        "hermes_agent.agent.models_dev.fetch_models_dev",
...
def test_switch_model_resolves_user_provider_credentials(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        "hermes_agent.hermes_cli.models.validate_requested_model",
...
def _run_user_provider_override_case(
    *,
    ...
    with patch("hermes_agent.hermes_cli.model_switch.resolve_alias", return_value=None), \
         ...
         patch("hermes_agent.hermes_cli.model_switch.normalize_model_for_provider", side_effect=lambda model, provider: model), \
         patch("hermes_agent.hermes_cli.models.validate_requested_model", return_value=_REJECTED_VALIDATION), \
         ...
         patch("hermes_agent.hermes_cli.runtime_provider.resolve_runtime_provider", return_value={"api_key": "***", "base_url": base_url, "api_mode": "anthropic_messages"}):
        return switch_model(
            raw_input=raw_input,
...
def test_section3_probes_no_key_endpoint_without_explicit_models(monkeypatch):
    ...
    monkeypatch.setattr("hermes_agent.hermes_cli.models.fetch_api_models", _fake_fetch)
...
def test_section3_skips_probe_when_no_key_but_explicit_models(monkeypatch):
    ...
    monkeypatch.setattr("hermes_agent.hermes_cli.models.fetch_api_models", _fail_fetch)

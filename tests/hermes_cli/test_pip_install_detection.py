...
def test_pip_install_detected_when_no_git_dir(tmp_path):
    ...
    with patch("hermes_agent.hermes_cli.config.get_managed_system", return_value=None), \
         patch("hermes_agent.hermes_cli.config.get_hermes_home", return_value=tmp_path):
...
def test_managed_install_takes_precedence(tmp_path):
    ...
    (tmp_path / ".git").mkdir()
    with patch("hermes_agent.hermes_cli.config.get_managed_system", return_value="NixOS"), \
         patch("hermes_agent.hermes_cli.config.get_hermes_home", return_value=tmp_path):
        from hermes_agent.hermes_cli.config import detect_install_method
        method = detect_install_method(project_root=tmp_path)
        assert method == "nixos"
...
def test_recommended_update_command_pip():
    ...
    from hermes_agent.hermes_cli.config import recommended_update_command_for_method
...
def test_container_without_stamp_is_not_docker(tmp_path):
    ...
    with patch("hermes_agent.hermes_cli.config.get_managed_system", return_value=None), \
         patch("hermes_agent.hermes_cli.config.get_hermes_home", return_value=tmp_path), \
         patch("hermes_agent.hermes_constants.is_container", return_value=True):
        from hermes_agent.hermes_cli.config import detect_install_method
...
def test_banner_warns_on_pip_install(tmp_path):
    ...
    from hermes_agent.hermes_cli import banner

    hh = tmp_path / ".hermes"
    hh.mkdir()
    (hh / ".install_method").write_text("pip\n")

    with patch("hermes_agent.hermes_cli.config.get_hermes_home", return_value=hh), \
         patch("hermes_agent.hermes_constants.get_hermes_home", return_value=hh):
        buf = io.StringIO()

...
from hermes_agent.hermes_cli.main import cmd_update, PROJECT_ROOT
...
class TestCmdUpdatePip:
    ...
    def test_update_pip_exports_virtualenv_from_sys_prefix(
        ...
        from hermes_agent.hermes_cli import main as hm
...
class TestCmdUpdateBranchFallback:
    ...
    def test_update_non_interactive_runs_safe_config_migrations(self, mock_args, capsys):
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "hermes_agent.hermes_cli.config.get_missing_env_vars", return_value=["MISSING_KEY"]
        ), patch(
            "hermes_agent.hermes_cli.config.get_missing_config_fields",
            return_value=[{"key": "new.option", "default": True}],
        ), patch("hermes_agent.hermes_cli.config.check_config_version", return_value=(1, 2)), patch(
            "hermes_agent.hermes_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": ["new.option"]},
        ), patch("hermes_agent.hermes_cli.main.sys") as mock_sys:
            ...
            from hermes_agent.hermes_cli.config import migrate_config
...
class TestCmdUpdateMigrationPrompt:
    ...
    def test_version_bump_only_applies_silently_without_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "hermes_agent.hermes_cli.config.get_missing_env_vars", return_value=[]
        ), patch(
            "hermes_agent.hermes_cli.config.get_missing_config_fields", return_value=[]
        ), patch(
            "hermes_agent.hermes_cli.config.check_config_version", return_value=(5, 24)
        ), patch(
            "hermes_agent.hermes_cli.config.migrate_config",
    ...
    def test_new_options_are_listed_by_name_before_prompt(
        ...
        with patch("shutil.which", return_value=None), patch(
            ...
            "hermes_agent.hermes_cli.config.get_missing_env_vars", return_value=env_items
        ), patch(
            "hermes_agent.hermes_cli.config.get_missing_config_fields", return_value=cfg_items
        ), patch(
            "hermes_agent.hermes_cli.config.check_config_version", return_value=(1, 24)
        ), patch(
            "hermes_agent.hermes_cli.config.migrate_config",
            return_value={"env_added": [], "config_added": [], "warnings": []},
        ), patch("hermes_agent.hermes_cli.main.sys") as mock_sys:
...
class TestCmdUpdateProfileSkillSync:
    ...
    def test_active_profile_included_in_skill_sync(
        ...
        with (
            patch("hermes_agent.hermes_cli.profiles.list_profiles", return_value=all_profiles),
            patch("hermes_agent.hermes_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("hermes_agent.tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)
    ...
    def test_single_profile_default_is_synced(
        ...
        with (
            patch("hermes_agent.hermes_cli.profiles.list_profiles", return_value=[default_p]),
            patch("hermes_agent.hermes_cli.profiles.seed_profile_skills", side_effect=fake_seed),
            patch("hermes_agent.tools.skills_sync.sync_skills", return_value=empty_sync),
        ):
            cmd_update(mock_args)

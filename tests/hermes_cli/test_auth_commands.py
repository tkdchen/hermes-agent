...
def test_auth_add_api_key_persists_manual_entry(tmp_path, monkeypatch):
    ...
    from hermes_agent.hermes_cli.auth_commands import auth_add_command
...
def test_auth_add_anthropic_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "hermes_agent.agent.anthropic_adapter.run_hermes_oauth_login_pure",
        lambda: {
            "access_token": token,
            "refresh_token": "refresh-token",
...
def test_auth_add_google_gemini_cli_sets_active_provider(tmp_path, monkeypatch):
    ...
    _write_auth_store(tmp_path, {"version": 1, "providers": {}})
    monkeypatch.setattr(
        "hermes_agent.agent.google_oauth.run_gemini_oauth_login_pure",
        lambda: {
...
def test_auth_add_qwen_oauth_sets_active_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "hermes_agent.hermes_cli.auth.resolve_qwen_runtime_credentials",
        lambda **kw: _fake_creds,
    )
    # Prevent _seed_from_singletons from calling the real Qwen CLI file path
    monkeypatch.setattr(
        "hermes_agent.agent.credential_pool._seed_from_singletons",
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "hermes_agent.hermes_cli.auth._nous_device_code_login",
        lambda **kwargs: {
...
def test_auth_add_minimax_oauth_starts_login_and_persists_pool_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "hermes_agent.hermes_cli.auth._minimax_oauth_login",
        lambda **kwargs: {
...
def test_auth_add_codex_oauth_keeps_distinct_pool_accounts(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("hermes_agent.hermes_cli.auth._codex_device_code_login", lambda: next(logins))
...
def test_codex_auth_status_reports_pool_only_credential(tmp_path, monkeypatch):
    ...
    from hermes_agent.hermes_cli.auth import get_codex_auth_status
...
def test_codex_runtime_pool_only_rate_limit_is_not_missing_auth(tmp_path, monkeypatch):
    ...
    from hermes_agent.hermes_cli.auth import AuthError, CODEX_RATE_LIMITED_CODE, resolve_codex_runtime_credentials
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        "hermes_agent.hermes_cli.auth._xai_oauth_loopback_login",
        lambda **kwargs: {
            "tokens": {
...
def test_auth_remove_reindexes_priorities(tmp_path, monkeypatch):
    ...
    from hermes_agent.hermes_cli.auth_commands import auth_remove_command
...
def test_auth_reset_clears_provider_statuses(tmp_path, monkeypatch, capsys):
    ...
    from hermes_agent.hermes_cli.auth_commands import auth_reset_command
...
def test_reset_config_provider_uses_atomic_yaml_write(tmp_path, monkeypatch):
    ...
    from hermes_agent.hermes_cli.auth import _reset_config_provider
    ...
    with patch("hermes_agent.hermes_cli.auth.atomic_yaml_write", side_effect=_boom) as mock_write:
...
def test_auth_list_does_not_call_mutating_select(monkeypatch, capsys):
    from hermes_agent.hermes_cli.auth_commands import auth_list_command
    ...
    monkeypatch.setattr(
        "hermes_agent.hermes_cli.auth_commands.load_pool",
...
def test_auth_list_shows_exhausted_cooldown(monkeypatch, capsys):
    ...
    monkeypatch.setattr("hermes_agent.hermes_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr("hermes_agent.hermes_cli.auth_commands.time.time", lambda: 1030.0)
...
def test_auth_list_prefers_explicit_reset_time(monkeypatch, capsys):
    ...
    monkeypatch.setattr("hermes_agent.hermes_cli.auth_commands.load_pool", lambda provider: _Pool())
    monkeypatch.setattr(
        "hermes_agent.hermes_cli.auth_commands.time.time",
...
def test_unsuppress_credential_source_clears_marker(tmp_path, monkeypatch):
    ...
    from hermes_agent.hermes_cli.auth import suppress_credential_source, unsuppress_credential_source, is_source_suppressed
...
def test_unsuppress_credential_source_returns_false_when_absent(tmp_path, monkeypatch):
    ...
    from hermes_agent.hermes_cli.auth import unsuppress_credential_source
...
def test_seed_from_singletons_respects_codex_suppression(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("hermes_agent.hermes_cli.auth._import_codex_cli_tokens", _fake_import)
...
def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    ...
    from types import SimpleNamespace
    from hermes_agent.hermes_cli.auth import is_source_suppressed
    from hermes_agent.hermes_cli.auth_commands import auth_add_command
...
def test_seed_from_singletons_respects_copilot_suppression(tmp_path, monkeypatch):
    ...
    import hermes_agent.hermes_cli.copilot_auth as ca
...
def test_seed_from_singletons_respects_qwen_suppression(tmp_path, monkeypatch):
    ...
    import hermes_agent.hermes_cli.auth as ha
...
def test_auth_remove_copilot_suppresses_all_variants(tmp_path, monkeypatch):
    ...
    with patch(
        ...
        "hermes_agent.hermes_cli.copilot_auth.get_copilot_api_token",

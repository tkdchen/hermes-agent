...
def _make_hermes_tree(root: Path) -> None:
    ...
    (root / "hermes_agent.hermes_state.db").write_bytes(b"fake-state")
    ...
    (root / "hermes-agent" / "hermes_agent.run_agent.py").write_text("# big file\n")
    ...
    (root / "hermes_agent.gateway.pid").write_text("12345")

    # Logs (should be included)
    (root / "logs").mkdir(exist_ok=True)
    (root / "logs" / "hermes_agent.agent.log").write_text("log line\n")
...
class TestShouldExclude:
    def test_excludes_hermes_agent(self):
        from hermes_agent.hermes_cli.backup import _should_exclude
    ...
    def test_excludes_pid_files(self):
        from hermes_agent.hermes_cli.backup import _should_exclude
        assert _should_exclude(Path("hermes_agent.gateway.pid"))
        assert _should_exclude(Path("hermes_agent.cron.pid"))
...
class TestBackup:
    ...
    def test_db_snapshots_staged_beside_output_zip(self, tmp_path, monkeypatch):
        ...
        import hermes_agent.hermes_cli.backup as backup_mod
        staged_dirs = []
...
class TestValidateBackupZip:
    ...
    def test_state_db_passes(self, tmp_path):
        ...
        from hermes_agent.hermes_cli.backup import _validate_backup_zip
    ...
    def test_old_wrong_db_name_fails(self, tmp_path):
        ...
        self._make_zip(zip_path, ["hermes_agent.hermes_state.db", "memory_store.db"])
        with zipfile.ZipFile(zip_path, "r") as zf:
            ok, reason = _validate_backup_zip(zf)
        assert not ok
...
class TestRoundTrip:
    def test_backup_then_import(self, tmp_path, monkeypatch):
        ...
        from hermes_agent.hermes_cli.backup import run_backup, run_import
        ...
        assert (dst_home / "logs" / "hermes_agent.agent.log").exists()
        ...
        assert not (dst_home / "hermes_agent.gateway.pid").exists()
...
class TestFormatSize:
    def test_bytes(self):
        from hermes_agent.hermes_cli.backup import _format_size
...
class TestValidation:
    ...
    def test_detect_prefix_hermes(self):
        ...
        import io
        from hermes_agent.hermes_cli.backup import _detect_prefix
...
class TestProfileRestoration:
    ...
    def test_import_without_profiles_module(self, tmp_path, monkeypatch):
        ...
        def fake_import(name, *a, **kw):
            if name == "hermes_agent.hermes_cli.profiles":
...
class TestSafeCopyDb:
    def test_copies_valid_database(self, tmp_path):
        from hermes_agent.hermes_cli.backup import _safe_copy_db
...
class TestQuickSnapshot:
    ...
    def test_list_snapshots(self, hermes_home):
        from hermes_agent.hermes_cli.backup import create_quick_snapshot, list_quick_snapshots
    ...
    def test_restore_config(self, hermes_home):
        from hermes_agent.hermes_cli.backup import create_quick_snapshot, restore_quick_snapshot
        snap_id = create_quick_snapshot(hermes_home=hermes_home)
    ...
    def test_restore_nonexistent(self, hermes_home):
        from hermes_agent.hermes_cli.backup import restore_quick_snapshot
        assert restore_quick_snapshot("nonexistent", hermes_home=hermes_home) is False

    def test_auto_prune(self, hermes_home):
        from hermes_agent.hermes_cli.backup import create_quick_snapshot, list_quick_snapshots, _QUICK_DEFAULT_KEEP
    ...
    def test_manual_prune(self, hermes_home):
        from hermes_agent.hermes_cli.backup import create_quick_snapshot, prune_quick_snapshots, list_quick_snapshots
        for i in range(10):
            create_quick_snapshot(label=f"s{i}", hermes_home=hermes_home)
...
class TestPreUpdateBackup:
    ...
    def test_backup_contents_match_full_backup(self, hermes_home):
        ...
        # pid files excluded
        assert "hermes_agent.gateway.pid" not in names
...
class TestRunPreUpdateBackup:
    ...
    def hermes_home(self, tmp_path, monkeypatch):
        ...
        for mod in list(__import__("sys").modules.keys()):
            if mod.startswith("hermes_agent.hermes_cli.config") or mod == "hermes_constants":
    ...
    def test_backup_flag_creates_backup(self, hermes_home, capsys):
        ...
        from hermes_agent.hermes_cli.main import _run_pre_update_backup
    ...
    def test_config_enabled_creates_backup(self, hermes_home, capsys):
        ...
        for mod in list(_sys.modules.keys()):
            if mod.startswith("hermes_agent.hermes_cli.config"):
...
class TestPreMigrationBackup:
    ...
    def test_creates_backup_under_backups_dir(self, hermes_home):
        from hermes_agent.hermes_cli.backup import create_pre_migration_backup
    ...
    def test_restorable_with_hermes_import(self, hermes_home, tmp_path):
        ...
        from hermes_agent.hermes_cli.backup import create_pre_migration_backup, _validate_backup_zip
        out = create_pre_migration_backup(hermes_home=hermes_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_does_not_touch_pre_update_backups(self, hermes_home):
        ...
        from hermes_agent.hermes_cli.backup import create_pre_update_backup, create_pre_migration_backup

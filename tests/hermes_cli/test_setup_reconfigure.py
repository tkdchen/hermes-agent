...
def _enter_existing_install_patches(stack, **extra):
    ...
    for target, kwargs in [
        ("hermes_agent.hermes_cli.setup.ensure_hermes_home", {}),
        ("hermes_agent.hermes_cli.setup.is_interactive_stdin", {"return_value": True}),
        ("hermes_agent.hermes_cli.config.is_managed", {"return_value": False}),
        ("hermes_agent.hermes_cli.setup.load_config", {"return_value": {}}),
        ("hermes_agent.hermes_cli.setup.save_config", {}),
        ("hermes_agent.hermes_cli.setup.get_env_value", {"return_value": None}),
        ("hermes_agent.hermes_cli.auth.get_active_provider", {"return_value": "openrouter"}),
        ("hermes_agent.hermes_cli.setup._print_setup_summary", {}),
        ("hermes_agent.hermes_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
...
def _enter_fresh_install_patches(stack, **extra):
    for target, kwargs in [
        ...
        ("hermes_agent.hermes_cli.setup.load_config", {"return_value": {}}),
        ("hermes_agent.hermes_cli.setup.save_config", {}),
        ("hermes_agent.hermes_cli.auth.get_active_provider", {"return_value": None}),
        ("hermes_agent.hermes_cli.setup.get_env_value", {"return_value": None}),
        ("hermes_agent.hermes_cli.setup._offer_openclaw_migration", {"return_value": False}),
    ]:
        stack.enter_context(patch(target, **kwargs))
    ...
    def test_bare_setup_runs_full_reconfigure_without_menu(self, existing_install):
        ...
        with ExitStack() as stack:
            m = _enter_existing_install_patches(
                stack,
                prompt_choice="hermes_agent.hermes_cli.setup.prompt_choice",
                quick="hermes_agent.hermes_cli.setup._run_quick_setup",
                model="hermes_agent.hermes_cli.setup.setup_model_provider",
                terminal="hermes_agent.hermes_cli.setup.setup_terminal_backend",
                agent="hermes_agent.hermes_cli.setup.setup_agent_settings",
                gateway="hermes_agent.hermes_cli.setup.setup_gateway",
                tools="hermes_agent.hermes_cli.setup.setup_tools",
            )
    ...
    def test_bare_setup_runs_first_time_flow(self, fresh_install):
        ...
        with ExitStack() as stack:
            m = _enter_fresh_install_patches(
                stack,
                prompt=("hermes_agent.hermes_cli.setup.prompt_choice", {"return_value": 0}),
                first="hermes_agent.hermes_cli.setup._run_first_time_quick_setup",
            )
            from hermes_agent.hermes_cli.setup import run_setup_wizard
            run_setup_wizard(args)
...
class TestArgparse:
    ...
    def test_reconfigure_flag_reaches_cmd_setup(self, monkeypatch):
        ...
        captured = {}
        monkeypatch.setattr(
            "hermes_agent.hermes_cli.setup.run_setup_wizard",

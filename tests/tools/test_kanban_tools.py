...
def test_kanban_tools_hidden_without_env_var(monkeypatch, tmp_path):
    ...
    import hermes_agent.tools.kanban_tools  # ensure registered
    from hermes_agent.tools.registry import invalidate_check_fn_cache, registry
...
def test_kanban_worker_env_overrides_profile_toolset_filter(monkeypatch, tmp_path):
    ...
    import hermes_agent.tools.kanban_tools  # ensure registered
    from hermes_agent.model_tools import _clear_tool_defs_cache, get_tool_definitions
...
def test_show_defaults_to_env_task_id(worker_env):
    from hermes_agent.tools import kanban_tools as kt
    out = kt._handle_show({})
    d = json.loads(out)
    assert "task" in d
...
def test_comment_schema_omits_author_override():
    ...
    """
    from hermes_agent.tools.kanban_tools import KANBAN_COMMENT_SCHEMA
...
def test_kanban_guidance_not_in_normal_prompt(monkeypatch, tmp_path):
    ...
    from hermes_agent.tools.registry import invalidate_check_fn_cache
    from hermes_agent.model_tools import _clear_tool_defs_cache
    invalidate_check_fn_cache()
    _clear_tool_defs_cache()

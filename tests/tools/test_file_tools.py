...
class TestWriteFileHandler:
    ...
    def test_permission_error_returns_error_json_without_error_log(self, mock_get, caplog):
        ...
        from hermes_agent.tools.file_tools import write_file_tool
        with caplog.at_level(logging.DEBUG, logger="hermes_agent.tools.file_tools"):
    ...
    def test_unexpected_exception_still_logs_error(self, mock_get, caplog):
        ...
        from hermes_agent.tools.file_tools import write_file_tool
        with caplog.at_level(logging.ERROR, logger="hermes_agent.tools.file_tools"):
            result = json.loads(write_file_tool("/tmp/out.txt", "data"))
    ...
    def test_explicit_empty_content_is_allowed(self):
        ...
        with patch("hermes_agent.tools.file_tools._get_file_ops") as mock_get:
            mock_ops = MagicMock()
            result_obj = MagicMock()
...
class TestSensitivePathCheck:
    ...
    def test_hermes_config_blocked_for_write_file(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("hermes_agent.tools.file_tools._hermes_config_resolved", str(fake_config))
        monkeypatch.setattr("hermes_agent.tools.file_tools._hermes_config_resolved_loaded", True)
    ...
    def test_system_path_still_blocked(self, monkeypatch):
        monkeypatch.setattr("hermes_agent.tools.file_tools._hermes_config_resolved", "/some/other/path")
        monkeypatch.setattr("hermes_agent.tools.file_tools._hermes_config_resolved_loaded", True)
    ...
    def test_normal_file_not_blocked(self, mock_get, monkeypatch):
        monkeypatch.setattr("hermes_agent.tools.file_tools._hermes_config_resolved", "/home/user/.hermes/config.yaml")
        monkeypatch.setattr("hermes_agent.tools.file_tools._hermes_config_resolved_loaded", True)
        mock_ops = MagicMock()
        result_obj = MagicMock()

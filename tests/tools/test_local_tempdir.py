...
class TestLocalTempDir:
    ...
    def test_falls_back_to_tempfile_when_tmp_missing(self, monkeypatch):
        ...
        with patch("hermes_agent.tools.environments.local.os.path.isdir", return_value=False), \
             patch("hermes_agent.tools.environments.local.os.access", return_value=False), \
             patch("hermes_agent.tools.environments.local.tempfile.gettempdir", return_value="/cache/tmp"), \

...
try:
    ...
    _tt_mod = sys.modules["hermes_agent.tools.terminal_tool"]
except ImportError:
...
class TestCwdHandling:
    ...
    def test_docker_default_cwd_maps_current_directory_when_enabled(self, monkeypatch):
        ...
        monkeypatch.setattr("hermes_agent.tools.terminal_tool.os.getcwd", lambda: "/home/user/project")
        monkeypatch.setenv("TERMINAL_ENV", "docker")
        monkeypatch.setenv("TERMINAL_DOCKER_MOUNT_CWD_TO_WORKSPACE", "true")
        monkeypatch.delenv("TERMINAL_CWD", raising=False)
        config = _tt_mod._get_env_config()
...
class TestEnsurepipFix:
    ...
    def test_modal_environment_creates_image_with_setup_commands(self):
        ...
        try:
            from hermes_agent.tools.environments.modal import _resolve_modal_image
        except ImportError:
            pytest.skip("hermes_agent.tools.environments.modal not importable")

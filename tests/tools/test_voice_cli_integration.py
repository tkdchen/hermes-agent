...
class TestStreamingTTSActivation:
    ...
    def test_activates_when_elevenlabs_and_sounddevice_available(self):
        ...
        try:
            ...
            pytest.skip("hermes_agent.tools.tts_tool not available")
...
class TestVprintForceParameter:
    ...
    def test_error_messages_use_force_in_run_agent(self):
        ...
        include force=True."""
        with open("hermes_agent.run_agent.py", "r") as f:
            source = f.read()
...
class TestCtrlCResetsContinuousMode:
    ...
    def test_ctrl_c_handler_resets_voice_continuous(self):
        ...
        with open("hermes_agent.cli.py") as f:
            source = f.read()
...
class TestEnableVoiceModeReal:
    ...
    @patch("hermes_agent.cli._cprint")
    @patch("hermes_agent.hermes_cli.config.load_config", return_value={"voice": {}})
    ...
    @patch("hermes_agent.cli._cprint")
    @patch("hermes_agent.hermes_cli.config.load_config", return_value={"voice": {"auto_tts": True}})
    @patch("hermes_agent.tools.voice_mode.check_voice_requirements",
           return_value={"available": True, "details": "OK"})
    @patch("hermes_agent.tools.voice_mode.detect_audio_environment",
           return_value={"available": True, "warnings": []})
    ...
    @patch("hermes_agent.cli._cprint")
    @patch("hermes_agent.hermes_cli.config.load_config", side_effect=Exception("broken config"))
    @patch("hermes_agent.tools.voice_mode.check_voice_requirements",
           return_value={"available": True, "details": "OK"})
    @patch("hermes_agent.tools.voice_mode.detect_audio_environment",
           return_value={"available": True, "warnings": []})
...
class TestVoiceBeepConfigReal:
    ...
    @patch("hermes_agent.hermes_cli.config.load_config", return_value={"voice": {"beep_enabled": False}})
    ...
    @patch(
        "hermes_agent.tools.voice_mode.check_voice_requirements",
        return_value={
            "available": True,
...
class TestVoiceSpeakResponseReal:
    ...
    def test_async_scheduling_clears_done_before_thread_start(self):
        ...
        with patch("hermes_agent.cli.threading.Thread", FakeThread):
...
class TestVoiceStopAndTranscribeReal:
    ...
    @patch("hermes_agent.cli.os.unlink")
    @patch("hermes_agent.cli.os.path.isfile", return_value=True)
    @patch("hermes_agent.hermes_cli.config.load_config", return_value={"stt": {}})
    ...
    @patch("hermes_agent.cli.os.unlink")
    @patch("hermes_agent.cli.os.path.isfile", return_value=True)
    @patch("hermes_agent.hermes_cli.config.load_config", return_value={"stt": {"model": "whisper-large-v3"}})
    @patch("hermes_agent.tools.voice_mode.transcribe_recording",

...
class TestModelSupportsVision:
    ...
    def test_exception_returns_false(self):
        agent = _make_agent()
        with patch("hermes_agent.agent.models_dev.get_model_capabilities", side_effect=RuntimeError("boom")):
            assert agent._model_supports_vision() is False
    ...
    def test_top_level_model_override_wins(self):
        ...
        with patch("hermes_agent.hermes_cli.config.load_config", return_value={"model": {"supports_vision": True}}), \
             patch("hermes_agent.agent.models_dev.get_model_capabilities", return_value=None):
            assert agent._model_supports_vision() is True
    ...
    def test_override_false_disables_vision_for_models_dev_models(self):
        ...
        fake_caps = MagicMock()
        fake_caps.supports_vision = True
        with patch("hermes_agent.hermes_cli.config.load_config", return_value={"model": {"supports_vision": False}}), \
             patch("hermes_agent.agent.models_dev.get_model_capabilities", return_value=fake_caps):
            assert agent._model_supports_vision() is False
